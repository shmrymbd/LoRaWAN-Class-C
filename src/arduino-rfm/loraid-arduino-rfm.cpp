/*
    ...
    *This fimrware supports
        ...
        *Channel hopping
        *
        *The following settings will be done
...
/****************************************************************************************************
*  Revision History
*  30 October 2020     Zaki      Separate NWS and APPS for easy maintenance on App
*                                Rename a function name to sendToGateway
*                                Restrict freq_idx to randomize 0-7 only
*****************************************************************************************************/
...
void LoRaIdClass::init(void)
{
    ...
    memset(NwkSKey, 0x00, 32);
    memset(AppSKey, 0x00, 32);
...
void LoRaIdClass::setNwkAccessKey(unsigned char *accessKey_in)
{
    ...
    memcpy(&NwkSKey_temp[0], &accessKey_in[0], 32);
...
void LoRaIdClass::setNwkAccessKey(char *accessKey_in)
{
    setNwkAccessKey((unsigned char *)accessKey_in);
}


void LoRaIdClass::setAppAccessKey(unsigned char *accessKey_in)
{
    memset(Session_Data.AppSKey, 0x30, sizeof(Session_Data.AppSKey));
    unsigned char AppSKey_temp[32];

    memset(AppSKey_temp, 0x30, sizeof(AppSKey_temp));
    memcpy(&AppSKey_temp[0], &accessKey_in[0], 32);
...
void LoRaIdClass::setAppAccessKey(char *accessKey_in)
{
    setAppAccessKey((unsigned char *)accessKey_in);
}
...
void LoRaIdClass::sendToGateway(unsigned char *data, unsigned int len, unsigned char confirm)
{
...
void LoRaIdClass::sendToGateway(String dataString, unsigned char confirm)
{
    unsigned char freq_idx;
...
void LoRaIdClass::sendToGateway(String dataString)
{
    unsigned char freq_idx;
...
void LoRaIdClass::sendToGateway(char *data, unsigned int len, unsigned char confirm)
{
    sendToGateway((unsigned char *)data, len, confirm);
}
